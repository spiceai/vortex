...
| 17 | Decoded segment cache | this change | Cached encoded bytes still have to be deserialized for each new reader tree, so high-request-rate scans keep paying decode CPU | `cargo test -p vortex-layout decoded_segment_cache_skips_the_encoded_segment_source --lib` | No |
