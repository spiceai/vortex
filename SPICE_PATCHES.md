...
| 16 | A timestamp value never goes through a Jiff span | this change | `Timestamp` validated a storage value, and rendered one, by building a Jiff span from it. A span's limits are not a timestamp's at either end. They run *past* the last instant, and the unchecked constructors abort outside them, so `i64::MAX` seconds panicked instead of being reported. They also stop one short of `i64::MIN` nanoseconds, which is 1677-09-21 — an instant a `timestamp[ns]` array holds and row 15's conversion produces, so the scalar refused a value the array accepted. `unpack_native` now checks `Timestamp::storage_range`, the single range row 15 converts into, and `Display` builds its instant with the reporting constructors and falls back to the raw count, since it has no way to report a failure. Reachable from any read, or any formatting, of such a value | `cargo test -p vortex-array --lib extension::datetime::timestamp` | Proposed — upstream defect, not Spice behaviour |
