...
| 16 | Timestamp validation reports instead of aborting | this change | `Timestamp::unpack_native` built its Jiff span with the unchecked constructors, which panic outside Jiff's *span* range — wider than its timestamp range — so a storage value between the two limits aborted rather than failing the range check below it. Reachable from any read of such a value | `cargo test -p vortex-array --lib an_out_of_range_storage_value_is_an_error_not_a_panic` | Proposed — upstream defect, not Spice behaviour |
