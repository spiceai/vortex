        ...
        loop {
            match &mut self.state {
                LazyScanState::Builder(builder) => {
                    ...
                    // Only `prepare` runs on the CPU pool. Split tasks are built lazily by the
                    // stream below, so that the configured concurrency — not the split count —
                    // bounds how many splits have registered reads.
                    let task = builder.session.handle().spawn_cpu(move || builder.prepare());
...
mod test {
    ...
    fn into_stream_bounds_registered_splits(#[case] concurrency: usize) -> VortexResult<()> {
        ...
        let mut ctx = array_session().create_execution_ctx();
        let probe = Arc::new(crate::scan::test::SplitProbe::default());
        let reader = Arc::new(crate::scan::test::ProbeLayoutReader::new(
            ROW_COUNT,
            Arc::clone(&probe),
        ));
        ...
        crate::scan::test::assert_bounded_scan(&values, ROW_COUNT, &probe, concurrency);
