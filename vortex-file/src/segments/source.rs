...
// The segment-read result channel is tokio's oneshot, re-exported by `vortex-io`, not
// `futures::channel::oneshot`: `ReadFuture` below is polled and then dropped on
// cancellation, and non-tokio oneshot receivers release a stored waker from inside
