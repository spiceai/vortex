...
[`init.sql`](./init.sql) generates a 2.5B-row three-column table (`id`, `col`, `col2`); the
numbered queries each pin down one scan behavior:
...
- [`2_sum-range.sql`](./2_sum-range.sql): a range filter on `id` that should prune most
  zone maps and only fully decode the two zones that overlap the bounds.
